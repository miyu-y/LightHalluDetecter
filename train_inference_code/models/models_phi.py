...
class TripletModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        anchor_input_ids = input_ids[0].to(device)
        positive_input_ids = input_ids[1].to(device)
        negative_input_ids = input_ids[2].to(device)

        anchor_attention_mask = attention_mask[0].to(device)
        positive_attention_mask = attention_mask[1].to(device)
        negative_attention_mask = attention_mask[2].to(device)
        ...
        text_logits = self.classifier(torch.cat([anchor_output, text_output], dim=1))

        classification_loss, triplet_loss = self.loss_function(
            anchor_output, positive_output, negative_output, text_logits, labels
        )

        loss = classification_loss + triplet_loss
        return ModelOutput(logits=text_logits, loss=loss, output=[anchor_output, text_output])
